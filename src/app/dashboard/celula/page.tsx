...
export default function DashboardCelulaPage() {
  ...
  function extrairBaseSemNumero(e: string) {
    const p = e.split(', ')
    if (p.length >= 2 && /^\d+[a-zA-Z]?$/.test(p[1].trim())) {
  ...
  useEffect(() => {
    ...
    async function carregarPagina() {
      ...
      const { data: perfilData } = await supabase
        .from('usuarios')
        .select('id, nome, is_lider, max_celulas')
        .eq('id', user.id)
        .single()
  ...
  function abrirNovaCelula() {
    ...
    if (celulas.length >= max) {
      showToast(`Você atingiu o limite de ${max} célula(s).`, 'error')
      return
    }
  ...
  async function buscarCoordenadas(enderecoBusca: string) {
    try {
      const consulta = `${enderecoBusca}, Brasil`
  ...
  function adicionarMembro() {
    ...
    if (membros.some((m) => m.nome.toLowerCase() === n.toLowerCase())) {
      showToast('Membro já cadastrado.', 'error')
      return
    }
  ...
  async function salvarMembros(celulaId: string) {
    ...
    if (validos.length > 0) {
      const { error: insErr } = await supabase
        .from('celula_membros')
        .insert(validos.map((m) => ({ celula_id: celulaId, nome: m.nome })))
  ...
  async function handleSalvarCelula() {
    ...
    if (qtd > 0 && membros.length !== qtd) {
      showToast(
        `Quantidade informada: ${qtd}, mas há ${membros.length} membro(s). Ajuste antes de salvar.`,
        'error'
      )
      return
    }
    ...
    try {
      const coords = await buscarCoordenadas(endFinal)
      const encontrouCoordenadas =
      ...
      if (!celulaAtiva) {
        ...
        if (error || !data) {
          showToast(error?.message || 'Erro ao criar célula.', 'error')
          setSalvando(false)
          return
        }
        ...
        if (encontrouCoordenadas) {
          ...
          showToast(
            'Célula criada, mas não foi possível localizar o endereço no mapa.',
            'warning'
          )
        }
      ...
      const { data, error } = await supabase
        ...
        .eq('id', celulaAtiva.id)
        .select()
        .single()

      if (error || !data) {
        showToast(error?.message || 'Erro ao atualizar célula.', 'error')
        setSalvando(false)
        return
      }
      ...
      if (encontrouCoordenadas) {
        ...
        showToast(
          'Célula atualizada, mas não foi possível localizar o endereço no mapa.',
          'warning'
        )
      }
    } catch (err) {
  ...
  const progresso =
    qtdInformada > 0 ? Math.min((membros.length / qtdInformada) * 100, 100) : 0
            ...
            <div className="flex items-center justify-between bg-gradient-to-r from-green-600 to-emerald-500 px-4 py-4 text-white sm:px-8 sm:py-6">
              <div>
                ...
                <p className="text-xs text-green-50 sm:text-sm">
                  {modo === 'lista'
                    ? `${celulas.length}/${maxCelulas} célula(s)`
                    : 'Preencha as informações da célula'}
                </p>
              </div>
                ...
                <div className="space-y-4">
                  {celulas.length === 0 ? (
                    ...
                    celulas.map((celula) => (
                      <div
                        key={celula.id}
                        className="rounded-2xl border border-slate-200 p-4 transition hover:shadow-md sm:p-5"
                      >
                  ...
                  >
                    {podecriarMais
                      ? '+ Criar nova célula'
                      : `Limite de ${maxCelulas} célula(s) atingido`}
                  </button>
                </div>
              )}
                ...
                <div className="rounded-2xl border border-slate-200 p-4 sm:p-6">
                  {celulaAtiva && (
                    <div className="mb-5 flex flex-wrap gap-x-4 gap-y-1 text-sm text-slate-500">
                      <p>
                        Código: <span className="font-semibold">{celulaAtiva.codigo}</span>
                      </p>
                      <p>
                        Atualizado:{' '}
                        <span className="font-semibold">
                          {new Date(celulaAtiva.atualizado_em).toLocaleString('pt-BR', {
                            timeZone: 'America/Sao_Paulo',
                            dateStyle: 'short',
                            timeStyle: 'short',
                          })}
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div>
                      ...
                      />
                      {!endereco && (
                        <p className="mt-1 text-xs text-slate-400">Busque o CEP primeiro.</p>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      {diaSemana && (
                        <p className="mt-2 text-xs text-slate-500">
                          Selecionado: <span className="font-semibold">{diaSemana}</span>
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className={`mt-0.5 text-xs font-medium sm:text-sm ${statusTexto}`}>
                          ...
                          {qtdInformada > 0 &&
                            membros.length < qtdInformada &&
                            ` — faltam ${qtdInformada - membros.length}`}
                        </p>
                      </div>
                    ...
                    {qtdInformada > 0 && (
                      <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-slate-200">
                        <div
                          className={`h-1.5 rounded-full transition-all duration-500 ${statusCor}`}
                              ...
                              {editando ? (
                                <div className="flex flex-col gap-2">
                                  ...
                                  <div className="flex gap-2">
                                    <button
                                      onClick={() => salvarEdicao(originalIndex)}
                                ...
                                <div className="flex flex-col gap-2">
                                  ...
                                  <div className="flex gap-2">
                                    <button
                                      onClick={() => confirmarRemocao(originalIndex)}
                                    ...
                                    <button
                                      onClick={() => setConfirmandoRemocaoIndex(null)}
                                      className="flex-1 rounded-lg bg-slate-200 py-2 text-xs font-semibold text-slate-700 transition hover:bg-slate-300 sm:flex-none sm:px-4"
                                    >
                                      Cancelar
                                    </button>
                                ...
                                <div className="flex items-center justify-between gap-2">
                                  <div className="flex min-w-0 items-center gap-2">
                                    {!buscaMembro && (
                                      <span className="shrink-0 select-none text-base text-slate-300">☰</span>
                                    )}
                                    <div className="min-w-0">
                                  ...
                                  <div className="flex shrink-0 gap-1.5">
                                    <button
                                      onClick={() => iniciarEdicao(originalIndex)}
                                    ...
                                    <button
                                      onClick={() => pedirConfirmacaoRemocao(originalIndex)}
