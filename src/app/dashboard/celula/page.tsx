...
export default function DashboardCelulaPage() {
  ...
  const [celulaAtiva, setCelulaAtiva] = useState<Celula | null>(null)
  ...
  function preencherFormComCelula(c: Celula, membrosData: Membro[]) {
    setNome(c.nome || '')
    setQuantidadePessoas(c.quantidade_pessoas?.toString() || '')
    setTipoCelula(c.tipo_celula || '')
    setDiaSemana(c.dia_semana || '')
    const endSalvo = c.endereco || ''
  ...
  function limparForm() {
    setNome(''); setCep(''); setNumero(''); setEndereco(''); setEnderecoCompleto('')
    setQuantidadePessoas(''); setTipoCelula(''); setDiaSemana('')
    setMembros([]); setNovoMembro(''); setBuscaMembro('')
    setEditandoIndex(null); setNomeEditando(''); setConfirmandoRemocaoIndex(null)
  }
  ...
  useEffect(() => {
    ...
    async function carregarPagina() {
      ...
      const { data: perfilData } = await supabase
        .from('usuarios').select('id, nome, is_lider, max_celulas').eq('id', user.id).single()
      if (!perfilData?.is_lider) { router.push('/dashboard'); return }
      setPerfil(perfilData)

      const { data: celulasData } = await supabase
        .from('celulas').select('*').eq('lider_id', user.id).order('criado_em', { ascending: true })
  ...
  // ── Abrir célula para editar ──────────────────────────────────────────────

  async function abrirCelula(celula: Celula) {
    setCelulaAtiva(celula)
    const { data: membrosData } = await supabase
      .from('celula_membros').select('id, nome')
      .eq('celula_id', celula.id).order('criado_em', { ascending: true })
    preencherFormComCelula(celula, membrosData || [])
    setModo('form')
  }

  function abrirNovaCelula() {
    const max = perfil?.max_celulas ?? 1
    if (celulas.length >= max) {
      showToast(`Você atingiu o limite de ${max} célula(s).`, 'error'); return
    }
    setCelulaAtiva(null)
    limparForm()
    setModo('form')
  ...
  async function buscarCep() {
    ...
    try {
      ...
      setEndereco(base); setCep(formatarCep(cepLimpo)); setNumero(''); setEnderecoCompleto(base)
  ...
  function adicionarMembro() {
    ...
    if (membros.some((m) => m.nome.toLowerCase() === n.toLowerCase())) { showToast('Membro já cadastrado.', 'error'); return }
    setMembros((prev) => [...prev, { nome: n }])
    setNovoMembro('')
  }

  function iniciarEdicao(index: number) { setEditandoIndex(index); setNomeEditando(membros[index].nome); setConfirmandoRemocaoIndex(null) }
  function salvarEdicao(index: number) {
    const n = nomeEditando.trim()
    if (!n) { showToast('O nome não pode ser vazio.', 'error'); return }
    if (membros.some((m, i) => i !== index && m.nome.toLowerCase() === n.toLowerCase())) { showToast('Nome já existe.', 'error'); return }
    setMembros((prev) => prev.map((m, i) => i === index ? { ...m, nome: n } : m))
    setEditandoIndex(null); setNomeEditando('')
  }
  function cancelarEdicao() { setEditandoIndex(null); setNomeEditando('') }
  function pedirConfirmacaoRemocao(index: number) { setConfirmandoRemocaoIndex(index); setEditandoIndex(null) }
  function confirmarRemocao(index: number) { setMembros((prev) => prev.filter((_, i) => i !== index)); setConfirmandoRemocaoIndex(null) }
  ...
  function onDragEnd() {
    ...
    const lista = [...membros]; const [movido] = lista.splice(from, 1); lista.splice(to, 0, movido)
    setMembros(lista); dragIndex.current = null; dragOverIndex.current = null
  }
  ...
  async function handleSalvarCelula() {
    ...
    try {
      ...
      const { data, error } = await supabase.from('celulas')
        ...
        .eq('id', celulaAtiva.id).select().single()
      if (error || !data) { showToast('Erro ao atualizar célula.', 'error'); setSalvando(false); return }
      await salvarMembros(celulaAtiva.id)
      setCelulas((prev) => prev.map((c) => c.id === data.id ? data : c))
      setCelulaAtiva(data)
  ...
  const maxCelulas = perfil?.max_celulas ?? 1
  const podecriarMais = celulas.length < maxCelulas
            ...
            <div className="flex items-center justify-between bg-gradient-to-r from-green-600 to-emerald-500 px-4 py-4 text-white sm:px-8 sm:py-6">
              <div>
                <h1 className="text-2xl font-bold sm:text-3xl">
                  {modo === 'lista' ? 'Minhas células' : celulaAtiva ? 'Editar célula' : 'Nova célula'}
                </h1>
                <p className="text-xs text-green-50 sm:text-sm">
                  {modo === 'lista' ? `${celulas.length}/${maxCelulas} célula(s)` : 'Preencha as informações da célula'}
                </p>
              </div>
              <button
            ...
            <div className="p-4 sm:p-8">
                ...
                <div className="space-y-4">
                  {celulas.length === 0 ? (
                    <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center">
                      <p className="text-slate-500">Você ainda não tem nenhuma célula cadastrada.</p>
                    </div>
                  ) : (
                    celulas.map((celula) => (
                      <div key={celula.id} className="rounded-2xl border border-slate-200 p-4 transition hover:shadow-md sm:p-5">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              ...
                              {celula.tipo_celula && (
                                <span className="rounded-full bg-green-100 px-2 py-0.5 text-xs text-green-700">{celula.tipo_celula}</span>
                              )}
                            </div>
                            <p className="mt-1 text-xs text-slate-500">
                              {celula.dia_semana && `${celula.dia_semana} • `}
                              {celula.quantidade_pessoas ? `${celula.quantidade_pessoas} pessoas` : 'Qtd. não informada'}
                            </p>
                            {celula.endereco && (
                              <p className="mt-0.5 truncate text-xs text-slate-400">{celula.endereco}</p>
                            )}
                            <p className="mt-1 text-xs text-slate-400">
                              Atualizado: {new Date(celula.atualizado_em).toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo', dateStyle: 'short', timeStyle: 'short' })}
                            </p>
                          </div>
                          <button onClick={() => abrirCelula(celula)}
                  ...
                  <button
                    onClick={abrirNovaCelula}
                    disabled={!podecriarMais}
                    className={`w-full rounded-2xl py-3.5 font-semibold text-white transition ${
                      podecriarMais
                        ? 'bg-gradient-to-r from-green-600 to-emerald-500 hover:from-green-700 hover:to-emerald-600'
                        : 'cursor-not-allowed bg-slate-300'
                    }`}
                  >
                    {podecriarMais ? '+ Criar nova célula' : `Limite de ${maxCelulas} célula(s) atingido`}
                  </button>
                </div>
              )}
              ...
              {modo === 'form' && (
                <div className="rounded-2xl border border-slate-200 p-4 sm:p-6">
                  {celulaAtiva && (
                    <div className="mb-5 flex flex-wrap gap-x-4 gap-y-1 text-sm text-slate-500">
                      <p>Código: <span className="font-semibold">{celulaAtiva.codigo}</span></p>
                      <p>Atualizado: <span className="font-semibold">
                        {new Date(celulaAtiva.atualizado_em).toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo', dateStyle: 'short', timeStyle: 'short' })}
                      </span></p>
                    </div>
                  )}
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div>
                      ...
                      <div className="mt-1 flex w-full gap-2">
                        <input value={cep} onChange={(e) => setCep(formatarCep(e.target.value))} placeholder="00000-000" maxLength={9}
                    ...
                    <div>
                      ...
                      <input value={numero} onChange={(e) => setNumero(e.target.value)} placeholder="Ex: 123" disabled={!endereco}
                    ...
                    <div>
                      ...
                      {diaSemana && <p className="mt-2 text-xs text-slate-500">Selecionado: <span className="font-semibold">{diaSemana}</span></p>}
                    </div>
                  </div>
                  ...
                  <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      {membros.length > 3 && (
                        <input value={buscaMembro} onChange={(e) => setBuscaMembro(e.target.value)} placeholder="Buscar..."
                    ...
                    {membros.length > 1 && !buscaMembro && <p className="mt-2 text-xs text-slate-400">☰ Arraste para reordenar</p>}
                        ...
                        membrosFiltrados.map(({ nome: nomeMembro, originalIndex }) => {
                          ...
                          return (
                            <div key={originalIndex}
                              ...
                              className={`rounded-xl border bg-white px-3 py-3 transition ${!buscaMembro ? 'cursor-grab active:cursor-grabbing' : ''} ${confirmando ? 'border-red-200 bg-red-50' : 'border-slate-200'}`}
                            >
                              {editando ? (
                                <div className="flex flex-col gap-2">
                                  <input autoFocus value={nomeEditando} onChange={(e) => setNomeEditando(e.target.value)}
                                    onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); salvarEdicao(originalIndex) } if (e.key === 'Escape') cancelarEdicao() }}
                                  ...
                                  <div className="flex gap-2">
                                    <button onClick={() => salvarEdicao(originalIndex)} className="flex-1 rounded-lg bg-green-600 py-2 text-xs font-semibold text-white transition hover:bg-green-700 sm:flex-none sm:px-4">Salvar</button>
                                    <button onClick={cancelarEdicao} className="flex-1 rounded-lg bg-slate-200 py-2 text-xs font-semibold text-slate-700 transition hover:bg-slate-300 sm:flex-none sm:px-4">Cancelar</button>
                                  </div>
                                </div>
                              ) : confirmando ? (
                                <div className="flex flex-col gap-2">
                                  <p className="text-sm font-medium text-red-700">Remover <span className="font-bold">{nomeMembro}</span>?</p>
                                  <div className="flex gap-2">
                                    <button onClick={() => confirmarRemocao(originalIndex)} className="flex-1 rounded-lg bg-red-500 py-2 text-xs font-semibold text-white transition hover:bg-red-600 sm:flex-none sm:px-4">Confirmar</button>
                                    <button onClick={() => setConfirmandoRemocaoIndex(null)} className="flex-1 rounded-lg bg-slate-200 py-2 text-xs font-semibold text-slate-700 transition hover:bg-slate-300 sm:flex-none sm:px-4">Cancelar</button>
                                  </div>
                                </div>
                              ) : (
                                <div className="flex items-center justify-between gap-2">
                                  <div className="flex min-w-0 items-center gap-2">
                                    ...
                                    <div className="min-w-0">
                                      ...
                                      <p className="truncate text-sm font-medium text-slate-800">{nomeMembro}</p>
                                    </div>
                                  </div>
                                  <div className="flex shrink-0 gap-1.5">
                                    <button onClick={() => iniciarEdicao(originalIndex)} className="rounded-lg bg-slate-100 px-2.5 py-1.5 text-xs font-semibold text-slate-700 transition hover:bg-slate-200 sm:px-3 sm:py-2">Editar</button>
                                    <button onClick={() => pedirConfirmacaoRemocao(originalIndex)} className="rounded-lg bg-red-50 px-2.5 py-1.5 text-xs font-semibold text-red-600 transition hover:bg-red-100 sm:px-3 sm:py-2">✕</button>
                                  </div>
                                </div>
                              )}
                            </div>
                    ...
                    <div className="mt-3 rounded-xl border border-slate-200 bg-white p-3">
                      ...
                      <div className="flex gap-2">
                        <input value={novoMembro} onChange={(e) => setNovoMembro(e.target.value)} placeholder="Nome do membro"
                  ...
                  <div className="mt-5">
                    <button onClick={handleSalvarCelula} disabled={salvando}
                      ...
                      {salvando ? 'Salvando...' : celulaAtiva ? 'Atualizar célula' : 'Criar célula'}
                    </button>
                  </div>
                </div>
              )}
