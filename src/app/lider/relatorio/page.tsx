...
export default function RelatorioPage() {
  ...
  useEffect(() => {
    async function carregarPagina() {
      ...
      if (perfilData.tipo !== 'lider') {
        alert('Você não tem acesso à área de relatórios do líder.')
        router.push('/login')
        return
      }
      ...
      if (celulaError) {
        alert('Erro ao carregar sua célula.')
        router.push('/lider')
        return
      }
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-100">
        <p className="text-slate-700 text-lg">Carregando relatório...</p>
      </div>
    )
  }
